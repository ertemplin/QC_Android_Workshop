...
public class MainActivity extends Activity {
	...
	public static final String TAG = "TweetColor";

	// See https://apps.twitter.com
	private static final String CONSUMER_KEY = "1mIdZhLOO1VFeV24wCvkxTnlR";
	private static final String CONSUMER_SECRET = "fls5sgvEfmMJAU7RYvWbmUvCGqP8p5q4fbOy13YFpDr8L7agiM";
	private static final String ACCESS_TOKEN = "2746707883-vQfMDFjgzjxOR2N1Q0MbKE0exi21wFE2lRVkfuu";
	private static final String ACCESS_TOKEN_SECRET = "NgnM1kJOejYIIeaYhPuCU01zhXsq5PJkVrOKWsy2od3Ee";
	...
	protected void onCreate(Bundle savedInstanceState) {
		 ...
		 * OnClickListener, such as having the activity implement the OnClickListener
		 * class, which is useful if your activity has a large number of buttons.
		 */
		colorButton.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				if (colors.size() > 0) {
					...
					Object[] c_temp = colors.toArray();
					int[] c = new int[c_temp.length];
					for (int i = 0; i < c_temp.length; i++) {
						c[i] = (Integer) c_temp[i];
					}
					intent.putExtra("colors", c);
				}
		...
		listButton.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				if (tweets.size() > 0) {
					Object[] t_temp = tweets.toArray();
					String[] t = new String[t_temp.length];
					for (int i = 0; i < t_temp.length; i++) {
						t[i] = (String) t_temp[i];
					}
					intent.putExtra("tweets", t);
				}
				startActivity(intent);
			}
		});
	 ...
	 * 
	 * This allows us to send a message containing the color received in a tweet.
	 * If the ColorDrawActivity is already active this will enable us to make
	 * sure it receives the color.
	 */
	 ...
	 * 
	 * This allows us to send a message containing the tweet received.
	 * If the TweetListActivity is already active this will enable us to make
	 * sure it receives the tweet to display immediately.
	 */
	...
	public void startStreaming() {
		UserStreamListener listener = new UserStreamListener() {
			 ...
			 * onStatus function
			 * 
			 * This is called when a tweet is sent to @QC_Purdue_Works.
			 * We want to find the color (if it exists) and display it.
			 */
			@Override
			public void onStatus(Status status) {
				...
				/*
				 *  We will display the tweet in the tweet list even if it
				 *  does not contain a color.
				 */
				sendMessage(tweet);
				tweets.add(0, tweet);
